function Invoke-CheckMouseInUse {
    ...
    Write-Verbose "[$(Get-Date -Format T)] Starting Invoke-CheckMouseUsage"
    Write-Verbose "[$(Get-Date -Format T)] Curent X: $($pos.X), Current Y: $($pos.Y)"
    while ((Get-Date) -lt $endTime) {
      Start-Sleep $Frequency
      $newPos = [System.Windows.Forms.Cursor]::Position
      Write-Verbose "[$(Get-Date -Format T)] Polled X: $($newPos.X), Polled Y: $($newPos.Y)"
